...
namespace Coral {
	...
	void ManagedObject::InvokeMethodInternal(std::string_view InMethodName, const void** InParameters, size_t InLength) const
	{
		...
		s_ManagedFunctions.InvokeMethodFptr(m_Handle, methodName.c_str(), InParameters, static_cast<int32_t>(InLength));
	}
	...
	{
		auto methodName = StringHelper::ConvertUtf8ToWide(InMethodName);
		s_ManagedFunctions.InvokeMethodRetFptr(m_Handle, methodName.c_str(), InParameters, static_cast<int32_t>(InLength), InResultStorage);
	}
